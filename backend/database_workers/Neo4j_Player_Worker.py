...
class Player_Worker:
    ...
    def add_personal_data_to_player(self, name, country, rank, rank_level, status, experience, play_style, previous_win_year, previous_libema_winner, age, height, favorite_shot, hand, personality_tags, personality_long, grass_advantage, career_high_rank, years_on_tour, coach, image_url, gender, country_code, career_high_year):
        with self.driver.session(database="neo4j") as session:
            result = session.execute_write(
                self._add_personal_data_to_player, name, country, rank, rank_level, status, experience, play_style, previous_win_year, previous_libema_winner, age, height, favorite_shot, hand, personality_tags, personality_long, grass_advantage, career_high_rank, years_on_tour, coach, image_url, gender, country_code, career_high_year)

            return result
        
    @staticmethod
    def _add_personal_data_to_player(tx, name, country, rank, rank_level, status, experience, play_style, previous_win_year, previous_libema_winner, age, height, favorite_shot, hand, personality_tags, personality_long, grass_advantage, career_high_rank, years_on_tour, coach, image_url, gender, country_code, career_high_year):
        query = """ MATCH (p { name: $name })
                    SET p.country = $country, p.rank = $rank, p.rank_level = $rank_level, p.status = $status, p.experience = $experience, p.play_style = $play_style, p.previous_win_year = $previous_win_year, p.previous_libema_winner = $previous_libema_winner, 
                ...
                """
        result = tx.run(query, name=name, country=country, rank=rank, rank_level=rank_level, status=status, experience=experience, play_style=play_style, previous_win_year=previous_win_year, previous_libema_winner=previous_libema_winner,
